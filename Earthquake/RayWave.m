...
%     for a=7
        ...
        for j=1:floor(length(seriesRX)/fitLength)-2
           ...
           if (abs(r2rx)>0) 
               if (abs(r2z)>0)
                   if(abs(cos(angle(a1(2)+i*a1(1))-angle((a2(2)+i*a2(1)))))>cos(phaseThreshold*pi/180) && abs((a2(2)+a2(1)*i)/1e3)>=ampThreshold) % Phase ampThreshold
        ...
        figure(2)
        line1=plot((1:length(FitData))/8,seriesRX*1e9,(1:length(FitData))/8,FitData/1e3,'--',(1:length(FitData))/8,ampThreshold*(1:length(FitData))./(1:length(FitData)));
        ylabel('Angle (nrad)')
        xlabel('Time (s)')
        legend('Data','Fit','Threshold')
        ...
        for j=1:floor(length(X)/len)-2
            if(max(abs(tempRX(floor(j*len/fitLength):floor((j+1)*len/fitLength))))>=ampThreshold) % Amplitude ampThreshold
                ...
                peak=crossX(floor(find(crossX==max(crossX))-1.5/freq):floor(find(crossX==max(crossX))+1.5/freq));
                peakLags=lags(floor(find(crossX==max(crossX))-1.5/freq):floor(find(crossX==max(crossX))+1.5/freq))';
%                 [myfit,s]=polyfit(peakLags,peak,2);
%                 delta_t_X=[delta_t_X -myfit(2)/(2*myfit(1))/sampf];
                
                f=fit(peakLags,10^12*peak,fittype('gauss1'));
                delta_t_X=[delta_t_X f.b1/8];

                figure(4)
%                 line3=plot(lags/8,crossX,peakLags/8,myfit(1)*peakLags.^2+myfit(2)*peakLags+myfit(3));
                line3=plot(lags/8,crossX,peakLags/8,f.a1*exp(-((peakLags-f.b1)/f.c1).^2)/10^12);
                ylabel('Cross Cor')
                xlabel('Lag (s)')
                legend('Data','Fit')
                ...
                peak=crossY(floor(find(crossY==max(crossY))-1.5/freq):floor(find(crossY==max(crossY))+1.5/freq));
                peakLags=lags(floor(find(crossY==max(crossY))-1.5/freq):floor(find(crossY==max(crossY))+1.5/freq))';
%                 [myfit,s]=polyfit(peakLags,peak,2);
%                 delta_t_Y=[delta_t_Y -myfit(2)/(2*myfit(1))/sampf];
                f=fit(peakLags,10^12*peak,fittype('gauss1'));
                delta_t_Y=[delta_t_Y f.b1/8];
            end
        end
        ...
        for k=0:1e4
            ...
            for p=1:min([length(btempZ) length(btempRX)])
                if(abs(btempRX(p))>=ampThreshold) % Amplitude ampThreshold
...
%         errVelA=[errVelA; std(4e3./sqrt((delta_t_X(find(not(isnan(delta_t_X))))).^2+(delta_t_Y(find(not(isnan(delta_t_Y)))))).^2)]  %std(tempBVelA')
        ...
        tempAng=mean(atan2(delta_t_Y(find(not(isnan(delta_t_Y)))),delta_t_X(find(not(isnan(delta_t_X)))))*180/pi)
        tempVel=mean(4e3./sqrt((delta_t_X(find(not(isnan(delta_t_X))))).^2+(delta_t_Y(find(not(isnan(delta_t_Y))))).^2))
        ang=[ang tempAng];
        velA=[velA abs(tempVel)];
        errVelS=[errVelS; std(abs(tempZ(find(not(isnan(tempZ))))./tempRX(find(not(isnan(tempRX)))).*sin(tempAng*pi/180)))];  %std(tempBVelS')
        errVelA=[errVelA; std(abs(4e3./sqrt((delta_t_X(find(not(isnan(delta_t_X))))).^2+(delta_t_Y(find(not(isnan(delta_t_Y)))))).^2))];  %std(tempBVelA')
        ...
        figure(5)
        line4=plot((1:length(pltV))*fitLength,pltV,(1:length(delta_t_X))*50*sampf/2,4e3./sqrt((delta_t_X).^2+(delta_t_Y).^2));
 ...
 %% Single Event Plotting
    ...
    hold on
    line4=errorbar(((cInd-1)*freqStep+startFreq),velS',-errVelS',errVelS');
    line5=errorbar(((cInd-1)*freqStep+startFreq),velA,-errVelA',errVelA','--');
    ylabel('Velocity (m/s)')
    xlabel('Frequency (Hz)')
    if plIndex==1
...
for k=1:length(avgV.keys)
    ...
    avgVP(cInd(k))=sum(abs(avgV(cInd(k))))/N;
    avgVelP(cInd(k))=sum(abs(avgVel(cInd(k))))/N;
    avgVErrP(cInd(k))=sqrt(sum(avgVErr(cInd(k)).^2))/N;
    avgVelErrP(cInd(k))=sqrt(sum(avgVelErr(cInd(k)).^2))/N;
end


fig2=figure(6)
hold on
l=errorbar(((cInd-1)*freqStep+startFreq)+10^-4,cell2mat(avgVP.values)/1000,-cell2mat(avgVErrP.values)/1000,cell2mat(avgVErrP.values)/1000);
ll=errorbar(((cInd-1)*freqStep+startFreq),cell2mat(avgVelP.values)/1000,-cell2mat(avgVelErrP.values)/1000,cell2mat(avgVelErrP.values)/1000,'--');
